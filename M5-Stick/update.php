...
if (!empty($_POST)) {
    ...
    if ($result->num_rows > 0) {
       ...
       // ตรวจสอบและอัปเดตค่าเฉลี่ยใน tbl_daily_avg
        $avg_sql = "SELECT * FROM tbl_daily_avg WHERE esp32_id = $espId AND date = '$currentDate'";
        ...
        if ($avgResult === false) {
            ...
            if ($avgResult->num_rows > 0) {
                ...
                $newAvgHeatIndex = ($avgData['heat_index'] + $heat_index) / 2;

                $update_avg_sql = "UPDATE tbl_daily_avg SET 
                    temp = '$newAvgTemp',
                    humidity = '$newAvgHumidity',
                    heat_index = '$newAvgHeatIndex'
                    WHERE esp32_id = $espId AND date = '$currentDate'";
                ...
                // เพิ่มข้อมูลใหม่
                $insert_avg_sql = "INSERT INTO tbl_daily_avg (esp32_id, temp, humidity, heat_index, date) VALUES (
                    '$espId',
                    '$temp',
                    '$hum',
                    '$heat_index',
